...
class HomeControllerSpecJapanese extends PlaySpec with BeforeAndAfter with BeforeAndAfterAll with GuiceOneAppPerSuite with DefaultAwaitTimeout with Injecting {
  ...
  val reference1 = Reference(url = "", surface = "証拠データが", surfaceIndex = 0, isWholeSentence = true,
  ...
  "The specification6" should {
    "returns an appropriate response" in {
      ...
      val paraphraseKnowledge2 = Knowledge(lang=lang, sentence=paraphrase2, extentInfoJson = "{}", knowledgeForTables=List(uploadTable(knowledgeForTablePara2Ng, transversalState)))
      val paraphraseKnowledge3 = Knowledge(lang=lang, sentence=paraphrase3, extentInfoJson = "{}", knowledgeForTables=List(uploadTable(knowledgeForTablePara3Ok, transversalState)))
      val paraphraseKnowledge4 = Knowledge(lang=lang, sentence=paraphrase4, extentInfoJson = "{}", knowledgeForTables=List(uploadTable(knowledgeForTablePara1Ng, transversalState)))
